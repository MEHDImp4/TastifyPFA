...
function App() {
  ...
  if (!isAuthenticated) {
    return <Login onSuccess={() => {}} title="Tastify KDS" />
  }
    ...
    <main className="min-h-screen bg-background text-foreground font-sans flex flex-col items-center justify-center gap-8 p-6">
      <div className="w-full max-w-[440px] animate-enter">
        <div className="bg-surface rounded-3xl border border-white/5 p-10 shadow-2xl text-center">
          <div className="w-24 h-24 bg-orange/10 rounded-2xl flex items-center justify-center mb-8 border border-orange/20 mx-auto relative">
          ...
          <p className="text-foreground-muted mb-6">Chef <span className="text-orange font-semibold">{user?.username}</span> en poste</p>
          
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange/10 border border-orange/20 text-[11px] font-bold text-orange uppercase tracking-[0.15em]">
