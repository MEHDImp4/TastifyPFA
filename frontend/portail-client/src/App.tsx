...
function App() {
  ...
  if (!isAuthenticated) {
    return <Login onSuccess={() => {}} title="Tastify Portail" />
  }
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white font-sans flex flex-col items-center justify-center gap-6 p-4">
      <div className="w-full max-w-md bg-[#141414] rounded-2xl border border-white/5 p-8 shadow-2xl text-center">
        <div className="w-16 h-16 bg-purple-600/10 rounded-xl flex items-center justify-center mb-6 border border-purple-500/20 mx-auto">
          <img src="/logo.svg" alt="Tastify" className="w-10 h-10" />
        </div>
        <h1 className="text-2xl font-bold tracking-tight text-purple-500">Tastify Portail</h1>
        <p className="text-gray-400 mt-2">Connecté en tant que <span className="text-white font-semibold">{user?.username}</span></p>
        <div className="mt-4 inline-block px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-xs font-bold text-purple-400 uppercase tracking-widest">
          {user?.role}
        </div>
      </div>
