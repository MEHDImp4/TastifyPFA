...
L'authentification repose sur \code{djangorestframework-simplejwt}. Le backend émet un access token de courte durée et un refresh token stocké dans un cookie HttpOnly \cite{simplejwt}. Le dépôt contient aussi une séparation staff/client pour éviter qu'une session ouverte sur un portail écrase l'autre.
...
Les figures~\ref{fig:capture-dashboard}, \ref{fig:capture-salle}, \ref{fig:capture-kds} et \ref{fig:capture-client} couvrent quatre espaces du projet : pilotage, salle, cuisine et client.
