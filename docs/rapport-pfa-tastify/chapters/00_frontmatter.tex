...
{\Large Rapport de projet fin d'année}\\[0.15cm]
{\large 3ème /4ème année}\\[0.15cm]
...
{\large \textbf{Annee UNIVERSITAIRE: 2023-2024}}\\[1.0cm]
...
\fbox{%
  ...
  \centering
  \vspace{0.4cm}
  {\Huge \bfseries TITRE DU THEME}\\[0.35cm]
  {\Large \bfseries Tastify}\\[0.2cm]
  {\large Application ERP de gestion de restaurant avec analyse de sentiment des avis clients}
  \vspace{0.4cm}
  \end{minipage}}
\end{center}
...
\textbf{Encadré par :} & \\
\textbf{Tuteur de l'école :} & Pr. Prénom et nom \\
\\[-0.15cm]
\textbf{Réalisé par :} & Prénom et nom de l'étudiant \\
\end{tabular}
...
Une partie importante du projet concerne l'analyse de sentiment des avis clients. Le code implémente un pipeline asynchrone qui analyse les commentaires au moyen de l'API Hugging Face lorsque le jeton est configuré, avec le modèle \code{nlptown/bert-base-multilingual-uncased-sentiment} pour les avis multilingues et \code{moussaKam/MARBERT-sentiment} pour les commentaires en arabe. En cas d'indisponibilité de l'API, le système utilise un secours local par mots-clés nommé \code{mots-cles-simple}. Cette fonctionnalité aide le gérant à suivre la satisfaction client sans interrompre l'expérience utilisateur.
...
A key part of the project is customer review sentiment analysis. The implemented pipeline processes reviews asynchronously through the Hugging Face API when a token is configured, using \code{nlptown/bert-base-multilingual-uncased-sentiment} for multilingual reviews and \code{moussaKam/MARBERT-sentiment} for Arabic comments. If the API is unavailable, the application falls back to a local keyword-based method named \code{mots-cles-simple}. This feature helps managers monitor customer satisfaction without blocking the user experience.
...
\chapter*{Informations à confirmer}
\addcontentsline{toc}{chapter}{Informations à confirmer}
La page de garde reprend le gabarit Word fourni. Certains champs de ce gabarit sont génériques ou contradictoires avec d'autres fichiers du dépôt. Ils ne sont donc pas inventés dans le rapport.

\begin{itemize}
  \item Le gabarit DOCX indique \textbf{Annee UNIVERSITAIRE: 2023-2024}. Le brouillon LaTeX présent dans le dépôt mentionnait 2025--2026. L'année officielle doit être confirmée avant dépôt final.
  \item Le gabarit DOCX contient \textbf{Tuteur de l'école : Pr. Prénom et nom}. Le nom exact de l'encadrant n'est pas vérifiable dans les fichiers exploités.
  \item Le gabarit DOCX contient \textbf{Prénom et nom de l'étudiant}. Le brouillon LaTeX du dépôt mentionnait Diouri Mehdi et El Kharrazi Ibtihal, mais le choix demandé impose de conserver la structure générique de la page de garde.
  \item Les captures présentes dans le dépôt sont réutilisées uniquement lorsqu'elles correspondent à des modules effectivement observés. Les captures manquantes sont listées en annexe au lieu d'être fabriquées.
\end{itemize}
