...
\begin{tikzpicture}[node distance=1.0cm and 0.8cm]
  \node[box, text width=2.9cm] (backoffice) {Back-office React\\port 3000};
  \node[box, below=of backoffice, text width=2.9cm] (client) {Portail client React\\port 3003};
  \node[box, right=1.6cm of $(backoffice)!0.5!(client)$, text width=3.2cm] (backend) {Backend Django ASGI\\REST + WebSocket\\port 8000};
  \node[store, right=1.4cm of backend, yshift=1.0cm, text width=2.7cm] (mysql) {MySQL 8\\volume persistant};
  \node[store, right=1.4cm of backend, yshift=-0.2cm, text width=2.7cm] (redis) {Redis 7\\DB WebSocket et broker};
  \node[box, right=1.4cm of backend, yshift=-1.4cm, text width=2.7cm] (worker) {Celery worker\\tâches de fond};
  \node[box, below=of worker, text width=2.7cm] (beat) {Celery Beat\\planification};

  \draw[flow] (backoffice) -- node[above, font=\scriptsize]{HTTP / WS} (backend);
  \draw[flow] (client) -- node[below, font=\scriptsize]{HTTP} (backend);
  \draw[flow] (backend) -- (mysql);
  \draw[flow] (backend) -- (redis);
  \draw[flow] (backend) -- (worker);
  \draw[flow] (beat) -- (worker);
  \draw[dashedflow] (worker) -- (mysql);
  \draw[dashedflow] (worker) -- (redis);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=0.9cm and 0.8cm]
  \node[box, text width=2.5cm] (user) {Utilisateur};
  \node[box, right=of user, text width=2.7cm] (front) {Frontend\\staff ou client};
  \node[box, right=of front, text width=2.8cm] (auth) {API auth\\Django};
  \node[store, right=of auth, text width=2.6cm] (db) {Base\\Utilisateur};

  \draw[flow] (user) -- node[above, font=\scriptsize]{identifiants} (front);
  \draw[flow] (front) -- node[above, font=\scriptsize]{login} (auth);
  \draw[flow] (auth) -- node[above, font=\scriptsize]{vérification} (db);
  \draw[flow] (auth) to[bend left=18] node[below, font=\scriptsize]{access token + cookie refresh} (front);
  \draw[dashedflow] (front) to[bend left=18] node[below, font=\scriptsize]{refresh} (auth);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=1.0cm and 0.75cm]
  \node[box, text width=2.6cm] (server) {Serveur\\prise de commande};
  \node[box, right=of server, text width=2.8cm] (api) {API commandes\\validation};
  \node[store, right=of api, text width=2.6cm] (db) {MySQL\\commande};
  \node[store, below=of api, text width=2.6cm] (redis) {Redis\\staff group};
  \node[box, right=of db, text width=2.6cm] (kds) {KDS cuisine\\affichage};
  \node[box, below=of kds, text width=2.6cm] (cook) {Cuisinier\\statut plat};

  \draw[flow] (server) -- (api);
  \draw[flow] (api) -- (db);
  \draw[flow] (api) -- (redis);
  \draw[dashedflow] (redis) -| (kds);
  \draw[flow] (cook) -- (api);
  \draw[dashedflow] (redis) -- (server);
\end{tikzpicture}
...
Les figures~\ref{fig:capture-dashboard}, \ref{fig:capture-salle}, \ref{fig:capture-kds} et \ref{fig:capture-client} couvrent les quatre espaces essentiels : pilotage, salle, cuisine et client. Les captures complémentaires à fournir sont listées en annexe.
...
\centering
\begin{tikzpicture}[node distance=0.9cm and 0.8cm]
  \node[box, text width=2.4cm] (compose) {Docker Compose};
  \node[store, below left=of compose, text width=2.2cm] (db) {\code{db}\\MySQL};
  \node[store, below=of compose, text width=2.2cm] (redis) {\code{redis}};
  \node[box, below right=of compose, text width=2.2cm] (backend) {\code{backend}\\Daphne};
  \node[box, below=of backend, text width=2.2cm] (worker) {\code{celery-worker}};
  \node[box, right=of worker, text width=2.2cm] (beat) {\code{celery-beat}};
  \node[box, right=of backend, text width=2.2cm] (staff) {\code{backoffice-app}};
  \node[box, right=of staff, text width=2.2cm] (client) {\code{client-app}};

  \draw[flow] (compose) -- (db);
  \draw[flow] (compose) -- (redis);
  \draw[flow] (compose) -- (backend);
  \draw[flow] (backend) -- (db);
  \draw[flow] (backend) -- (redis);
  \draw[flow] (worker) -- (redis);
  \draw[flow] (worker) -- (db);
  \draw[flow] (beat) -- (worker);
  \draw[flow] (staff) -- (backend);
  \draw[flow] (client) -- (backend);
\end{tikzpicture}
