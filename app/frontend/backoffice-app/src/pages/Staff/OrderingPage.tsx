...
export const OrderingPage: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-surface-main p-0 selection:bg-primary/20 selection:text-primary font-body overflow-hidden">
      ...
      {/* Contextual Top Bar */}
      <header className="flex-none flex items-center justify-between border-b border-outline-variant bg-surface-main px-4 md:px-staff-margin py-unit-sm h-16">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="font-serif text-lg md:text-xl font-black text-on-surface tracking-widest uppercase truncate max-w-[120px] md:max-w-none">
             Table {table?.numero || '??'}
          </div>
          {!isMobile && (
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <div className="relative group">
            ...
            <input 
              ...
              className="w-32 md:w-64 h-10 bg-surface-container-low border border-outline-variant pl-9 md:pl-10 pr-4 rounded font-sans text-xs font-bold text-on-surface focus:border-primary outline-none transition-all placeholder:text-on-surface-variant/40"
            />
          </div>
          {isMobile && (
            <button 
              onClick={() => setShowCart(!showCart)}
              className={`p-2 rounded relative transition-all ${showCart ? 'bg-primary text-on-primary' : 'bg-surface-container border border-outline-variant text-on-surface-variant'}`}
            >
              {showCart ? <UtensilsCrossed className="w-5 h-5" /> : <ReceiptText className="w-5 h-5" />}
              {cart.length > 0 && !showCart && (
                <span className="absolute -top-1 -right-1 bg-error text-on-error text-[10px] font-black w-4 h-4 rounded-full flex items-center justify-center border border-surface-main">
                  {cart.length}
                </span>
              )}
            </button>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <section 
          ...
          className={`
            flex-[7] h-full border-r border-outline-variant bg-surface-container-lowest flex flex-col min-w-0 transition-all duration-300
        ...
        >
          ...
          <div className="flex-none flex overflow-x-auto border-b border-outline-variant bg-surface-container py-2 px-4 md:px-staff-gutter gap-2 custom-scrollbar">
            <button
          ...
          {/* Menu Items Grid */}
          <div className="flex-1 overflow-y-auto p-4 md:p-staff-margin grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-unit-md auto-rows-max custom-scrollbar">
            {filteredPlats.map(plat => (
              <button
                ...
                onClick={() => {
                  addToCart(plat);
                  if (isMobile) {
                    toast.success(`${plat.nom} ajouté`, { duration: 1000, position: 'bottom-center' });
                  }
                }}
                className={`
                  group bg-surface-container border border-outline-variant rounded-lg p-3 md:p-unit-md text-left flex flex-col justify-between h-32 md:h-36 transition-all relative overflow-hidden
              ...
              >
                <div className="flex flex-col gap-1">
                  <span className="font-body text-[14px] md:text-[15px] font-bold text-on-surface leading-tight line-clamp-2">{plat.nom}</span>
                  <span className="font-sans text-[9px] md:text-[10px] font-black text-on-surface-variant/60 uppercase tracking-widest">
                    {categories.find(c => c.id === plat.categorie)?.nom}
                  </span>
                </div>
                <div className="flex items-end justify-between">
                  <span className="font-sans text-[14px] md:text-[16px] font-black text-primary">{parseFloat(plat.prix).toFixed(0)} DH</span>
                ...
                {!plat.est_disponible && (
                   <span className="absolute top-2 right-2 px-2 py-0.5 bg-error text-on-error font-sans text-[8px] md:text-[9px] font-black uppercase tracking-tighter rounded">SOLD OUT</span>
                )}
              </button>
            ))}
          </div>
        ...
        <section 
          ...
          className={`
            flex-[3] h-full bg-surface-container-lowest flex flex-col shadow-2xl z-10 transition-all duration-300
            ${isMobile ? (showCart ? 'w-full translate-x-0 relative' : 'w-full translate-x-[100%] absolute inset-0 opacity-0 pointer-events-none') : 'min-w-[320px] translate-x-0 relative'}
          `}
        >
          {/* Ticket Header */}
      ...
      {/* Mobile Footer / Shortcut */}
      {isMobile && !showCart && cart.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-3rem)] max-w-sm">
           <button 
             onClick={() => setShowCart(true)}
             className="w-full h-14 bg-primary text-on-primary rounded-full shadow-2xl flex items-center justify-between px-8 font-sans text-[11px] font-black uppercase tracking-[0.2em] animate-in slide-in-from-bottom-10 fade-in duration-500"
           >
             <span className="flex items-center gap-3">
               <ReceiptText className="w-4 h-4" />
               View Ticket ({cart.length})
             </span>
             <span className="bg-on-primary/10 px-3 py-1 rounded-full border border-on-primary/20">{cartTotal.toFixed(0)} DH</span>
           </button>
        </div>
      )}
    </div>
