...
export const SallePage: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-background -m-4 overflow-hidden selection:bg-primary/20 selection:text-primary font-body">
      <h1 className="sr-only" style={{ position: 'absolute', left: '-9999px', top: '-9999px' }}>Architectural Floor Plan</h1>
        ...
        <div className="flex flex-wrap items-center gap-6">
          <div className="flex bg-surface-container-low p-1.5 rounded-2xl border border-outline-variant/30">
             <button 
               ...
               className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl font-sans text-[11px] font-black uppercase tracking-widest transition-all ${viewMode === 'plan' ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high'}`}
             >
                <MapIcon className="w-4 h-4" /> Plan
             </button>
             <button 
               onClick={() => setViewMode('grid')}
               className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl font-sans text-[11px] font-black uppercase tracking-widest transition-all ${viewMode === 'grid' ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high'}`}
             >
        ...
        <div className="flex items-center gap-3">
          {role === 'GERANT' && isEditMode && viewMode === 'plan' && (
            <div className="flex items-center gap-3 pr-4 border-r border-outline-variant/20">
              <button
                ...
                className="px-5 py-2.5 bg-surface-container border border-outline-variant text-on-surface font-sans text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-primary hover:text-on-primary transition-all flex items-center gap-2.5"
              >
          ...
          {role === 'GERANT' && viewMode === 'plan' && (
            <button 
              ...
              className={`px-6 py-3 rounded-xl font-sans text-[11px] font-black uppercase tracking-[0.3em] flex items-center gap-3 transition-all ${isEditMode ? 'bg-primary text-on-primary border-2 border-primary hover:scale-[1.02]' : 'bg-surface-container-low border-2 border-outline-variant/30 text-on-surface hover:bg-surface-container-high'}`}
            >
                                ...
                                {tables.filter(t => t.est_active).map(table => (
                                    ...
                                    >
                                        ...
                                        {table.statut === 'ENCAISSEMENT' && (
                                           <div className="absolute -top-4 -right-4 bg-error text-on-error p-2.5 rounded-full border-2 border-surface"><DollarSign className="w-5 h-5" strokeWidth={3} /></div>
                                        )}
                                    </motion.div>
                                ))}
                            </div>
      ...
      <AnimatePresence>
        {(selectedTableForEdit || isAddingTable) && (
            <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-8">
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-surface-container border border-outline-variant p-12 max-w-md w-full rounded-[3rem] relative overflow-hidden">
                    ...
                    <div className="space-y-10">
                        <div className="space-y-3">
                            ...
                            <input type="number" value={formData.numero} onChange={(e) => setFormData({ ...formData, numero: e.target.value })} className="w-full h-20 px-8 bg-background border-2 border-outline-variant rounded-2xl font-mono text-3xl font-black text-primary focus:border-primary transition-all" />
                        </div>
                        <div className="space-y-3">
                            ...
                            <input type="number" value={formData.capacite} onChange={(e) => setFormData({ ...formData, capacite: e.target.value })} className="w-full h-20 px-8 bg-background border-2 border-outline-variant rounded-2xl font-mono text-3xl font-black text-on-surface focus:border-primary transition-all" />
                        </div>
                    </div>
                    <div className="flex gap-6 mt-16">
                        {selectedTableForEdit && <button onClick={() => setDeleteConfig({ isOpen: true, id: selectedTableForEdit.id, type: 'table' })} className="w-20 h-20 flex items-center justify-center border-2 border-error/30 text-error hover:bg-error hover:text-on-error rounded-2xl transition-all active:scale-90"><Trash2 className="w-8 h-8" /></button>}
                        <button onClick={async () => { try { if (selectedTableForEdit) { await salleApi.updateTable(selectedTableForEdit.id, { numero: parseInt(formData.numero), capacite: parseInt(formData.capacite) }); } else { await salleApi.createTable({ numero: parseInt(formData.numero), capacite: parseInt(formData.capacite), pos_x: 50, pos_y: 50, statut: 'LIBRE', est_active: true }); } fetchData(); setSelectedTableForEdit(null); setIsAddingTable(false); } catch (err) { toast.error('Erreur'); } }} className="flex-1 h-20 bg-primary text-on-primary font-sans text-xs font-black uppercase tracking-[0.5em] rounded-2xl hover:brightness-110 active:scale-95">Valider</button>
                    </div>
                </motion.div>
            </div>
        )}
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-surface-container border border-outline-variant p-12 max-w-md w-full rounded-[3rem] relative overflow-hidden">
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            ...
                            <input type="text" value={textFormData.texte} onChange={(e) => setTextFormData({ ...textFormData, texte: e.target.value })} className="w-full h-20 px-8 bg-background border-2 border-outline-variant rounded-2xl font-serif text-2xl font-black text-on-surface focus:border-primary transition-all uppercase" />
                        </div>
                    </div>
                    <div className="flex gap-6 mt-16">
                        {selectedTextForEdit && <button onClick={() => setDeleteConfig({ isOpen: true, id: selectedTextForEdit.id, type: 'text' })} className="w-20 h-20 flex items-center justify-center border-2 border-error/30 text-error hover:bg-error hover:text-on-error rounded-2xl transition-all active:scale-90"><Trash2 className="w-8 h-8" /></button>}
                        <button onClick={async () => { try { if (selectedTextForEdit) { await salleApi.updatePlanText(selectedTextForEdit.id, { texte: textFormData.texte }); } else { await salleApi.createPlanText({ texte: textFormData.texte, pos_x: 50, pos_y: 50, est_active: true }); } fetchData(); setSelectedTextForEdit(null); setIsAddingText(false); } catch (err) { toast.error('Erreur'); } }} className="flex-1 h-20 bg-primary text-on-primary font-sans text-xs font-black uppercase tracking-[0.5em] rounded-2xl hover:brightness-110 active:scale-95">Valider</button>
                    </div>
                </motion.div>
            </div>
        )}
