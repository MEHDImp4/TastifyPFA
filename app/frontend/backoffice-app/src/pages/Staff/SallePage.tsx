...
export const SallePage: React.FC = () => {
  ...
  const getStatutColor = (statut: Table['statut']) => {
    switch (statut) {
      ...
      case 'OCCUPEE': return 'bg-primary text-white border-primary/20 shadow-lg shadow-primary/10 shadow-primary/20';
      case 'RESERVEE': return 'bg-secondary text-white border-secondary/20';
      case 'ENCAISSEMENT': return 'bg-error text-white border-error/20 shadow-lg shadow-primary/10 shadow-error/20';
  ...
  return (
    <div className="max-w-[1600px] mx-auto h-[calc(100dvh-10rem)] flex flex-col animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-3 shrink-0">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-on-surface font-sans">Architectural Floor Plan</h1>
               ...
               <div className="flex items-center gap-3">
                 {isEditMode && (
                     ...
                     <button
                        ...
                        className="flex items-center gap-2 px-4 py-3 rounded-xl font-bold transition-all duration-300 active:scale-95 bg-surface-container-high text-on-surface hover:bg-surface-container-highest"
                     >
                 ...
                 <button 
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all duration-300 active:scale-95 ${isEditMode ? 'bg-primary text-white shadow-lg shadow-primary/10 shadow-primary/20' : 'bg-white border border-surface-container-high text-on-surface hover:bg-surface-container-low'}`}
                 >
                     <Move className="w-5 h-5"  strokeWidth={2}/>
                     <span className="font-sans text-sm">{isEditMode ? 'Save Layout' : 'Modify Layout'}</span>
                 </button>
               </div>
           )}
           <div className="flex items-center gap-3 px-8 py-2 glass rounded-xl border border-surface-container-high shadow-lg shadow-primary/10">
             <div className="flex items-center gap-2.5">
               <div className="w-2.5 h-2.5 rounded-xl border border-surface-container-highest bg-white" />
               <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest font-sans font-sans">Available</span>
             </div>
             <div className="flex items-center gap-2.5">
               <div className="w-2.5 h-2.5 rounded-xl bg-primary" />
               <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest font-sans font-sans">Occupied</span>
             </div>
             <div className="flex items-center gap-2.5">
               <div className="w-2.5 h-2.5 rounded-xl bg-secondary" />
               <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest font-sans font-sans">Reserved</span>
             </div>
             <div className="flex items-center gap-2.5">
               <div className="w-2.5 h-2.5 rounded-xl bg-error" />
               <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest font-sans font-sans">Payment</span>
             </div>
           </div>
        </div>
      </div>
            ...
            >
                ...
                {planTexts.filter(t => t.est_active).map((textObj) => (
                    <div
                        ...
                        className={`
                            ...
                            ${draggingItem?.type === 'text' && draggingItem.id === textObj.id ? 'z-50 ring-4 ring-primary/20 scale-110 shadow-lg shadow-primary/10' : 'z-0'}
                        `}
                    >
                        <span className="text-xl font-display-accent italic tracking-tighter text-on-surface-variant/50 whitespace-nowrap">{textObj.texte}</span>
                    </div>
                ))}
                ...
                {tables.filter(t => t.est_active).map((table) => (
                    <div
                        ...
                        className={`
                            absolute w-20 h-20 sm:w-24 sm:h-24 flex flex-col items-center justify-center gap-1 rounded-xl border-2 transition-all duration-500 group
                            ...
                            ${draggingItem?.type === 'table' && draggingItem.id === table.id ? 'z-50 ring-4 ring-primary/20 scale-110 shadow-lg shadow-primary/10' : 'z-10 shadow-lg shadow-primary/10'}
                        `}
                    >
                        <div className="text-lg sm:text-xl font-bold font-sans tracking-tighter leading-none">{table.numero}</div>
                        <div className={`flex items-center gap-1 px-2 py-0.5 rounded-xl text-[9px] font-bold uppercase tracking-widest font-sans ${table.statut === 'LIBRE' ? 'bg-surface-container text-on-surface-variant' : 'bg-black/10 text-white'}`}>
                        ...
                        {table.statut === 'ENCAISSEMENT' && !isEditMode && (
                            <div className="absolute -top-2 -right-2">
                                <span className="relative flex h-5 w-5">
                                    <span className="animate-ping absolute inline-flex h-full w-full rounded-xl bg-error opacity-75"></span>
                                    <span className="relative inline-flex rounded-xl h-5 w-5 bg-error border-2 border-white"></span>
                                </span>
                            </div>
                        )}
      ...
      {(selectedTableForEdit || isAddingTable) && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full shadow-lg shadow-primary/10 animate-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-lg font-bold text-on-surface font-display">{selectedTableForEdit ? 'Edit Table' : 'Add New Table'}</h2>
                    <button onClick={() => { setSelectedTableForEdit(null); setIsAddingTable(false); }} className="p-2 hover:bg-surface-container rounded-xl text-on-surface-variant transition-colors">
                ...
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-bold text-on-surface-variant mb-2 font-sans uppercase tracking-wider font-sans">Table Number</label>
                        <input 
                            type="number" 
                            value={formData.numero}
                            onChange={(e) => setFormData({ ...formData, numero: e.target.value })}
                            className="w-full px-4 py-3 rounded-xl bg-surface-container-low border-2 border-transparent focus:border-primary focus:bg-white transition-all outline-none font-sans text-sm font-bold"
                            placeholder="e.g. 12"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-on-surface-variant mb-2 font-sans uppercase tracking-wider font-sans">Capacity</label>
                        <input 
                            type="number" 
                ...
                <div className="flex items-center gap-2 mt-8">
                    ...
                    <button 
                        ...
                        className="flex-1 py-3 bg-primary text-white font-bold rounded-xl hover:shadow-lg shadow-primary/10 hover:shadow-primary/30 transition-all active:scale-95"
                    >
                        Save Changes
                    </button>
                </div>
      ...
      {(selectedTextForEdit || isAddingText) && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl p-5 max-w-md w-full shadow-lg shadow-primary/10 animate-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-lg font-bold text-on-surface font-display">{selectedTextForEdit ? 'Edit Text' : 'Add New Text'}</h2>
                    <button onClick={() => { setSelectedTextForEdit(null); setIsAddingText(false); }} className="p-2 hover:bg-surface-container rounded-xl text-on-surface-variant transition-colors">
                        <X className="w-6 h-6"  strokeWidth={2}/>
                    </button>
                </div>
                ...
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-bold text-on-surface-variant mb-2 font-sans uppercase tracking-wider font-sans">Text Content</label>
                        <input 
                            type="text" 
