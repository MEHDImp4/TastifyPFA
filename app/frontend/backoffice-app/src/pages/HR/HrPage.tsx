        ...
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-8 bg-surface border border-outline px-6 py-2.5 rounded-lg">
                <div className="text-center border-r border-outline pr-8">
                    ...
                    <p className="text-lg font-bold text-on-background mt-0.5">{employes.filter(e => e.user_details?.is_active ?? true).length}</p>
                </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {paginatedEmployes.length > 0 ? paginatedEmployes.map((emp) => (
                ...
                >
                  ...
                  <div className="col-span-2 flex justify-center">
                    <div className={`flex items-center gap-2 px-3 py-1 rounded border ${(emp.user_details?.is_active ?? true) ? 'bg-success/5 border-success/30 text-success' : 'bg-surface-container-low border-outline text-on-surface-variant'}`}>
                      <div className={`w-1 h-1 rounded-full ${(emp.user_details?.is_active ?? true) ? 'bg-success' : 'bg-outline-variant'}`} />
                      <span className="text-[8px] font-bold uppercase tracking-widest">{(emp.user_details?.is_active ?? true) ? 'ACTIF' : 'OFF'}</span>
                    </div>
                  </div>
