...
describe('HrPage component', () => {
  ...
  it('renders loading state initially', () => {
    ...
    vi.mocked(hrApi.getEmployes).mockResolvedValue({ data: [], status: 200, statusText: 'OK', headers: {}, config: {} as any });
    vi.mocked(hrApi.getShifts).mockResolvedValue({ data: [], status: 200, statusText: 'OK', headers: {}, config: {} as any });
    render(<HrPage />);
  ...
  it('renders employees list with user_details correctly', async () => {
    const mockEmployes: Employe[] = [
      {
        id: 1, user: 10, salaire: '2500.00', poste: 'GERANT',
        telephone: '0612345678', cin: 'AB12345', date_embauche: '2026-01-01',
        user_details: {
          id: 10, username: 'chef_gerant', first_name: 'John', last_name: 'Doe',
          email: 'john.doe@tastify.com', role: 'GERANT', is_active: true,
        },
      },
    ];

    vi.mocked(hrApi.getEmployesPage).mockResolvedValue({
      data: { count: mockEmployes.length, next: null, previous: null, results: mockEmployes },
      status: 200, statusText: 'OK', headers: {}, config: {} as any,
    });
    vi.mocked(hrApi.getEmployes).mockResolvedValue({ data: mockEmployes, status: 200, statusText: 'OK', headers: {}, config: {} as any });
  ...
  it('falls back to root-level employee properties when user_details is missing', async () => {
    const mockEmployes: Employe[] = [
      {
        id: 2, user: 20, salaire: '2000.00', poste: 'SERVEUR',
        telephone: '0687654321', cin: 'CD98765', date_embauche: '2026-02-02',
        username: 'alice_root', first_name: 'Alice', last_name: 'Smith',
        email: 'alice@tastify.com', is_active: false,
      },
    ];
