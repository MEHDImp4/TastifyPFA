    ...
    <div className="flex-1 bg-background font-body selection:bg-primary/20 overflow-y-auto custom-scrollbar">
      <main className="max-w-7xl mx-auto px-client-margin py-12 md:py-24">
        ...
        <div className="flex items-center gap-6 mb-16 border-b border-outline-variant pb-10">
            <button aria-label="Retour à la carte" onClick={() => navigate('/menu')} className="w-12 h-12 bg-surface-container-low rounded-xl hover:bg-surface-container-high transition-all border border-outline-variant/30 flex items-center justify-center text-on-surface"><ChevronLeft className="w-5 h-5" /></button>
            <div>
               ...
               <p className="font-sans text-[10px] font-black text-on-surface-variant uppercase tracking-[0.4em] mt-3">Validation de la commande</p>
            </div>
        </div>
            ...
            <div className="lg:col-span-8 space-y-12">
                        ...
                        {items.map(item => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <button aria-label={`Retirer ${item.plat.nom} du panier`} onClick={() => removeItem(item.plat.id)} className="p-3 text-on-surface-variant hover:text-error transition-all active:scale-75"><Trash2 className="w-5 h-5" /></button>
                                </div>
                            </motion.div>
                        ))}
                    </AnimatePresence>
                ...
                <div className="p-8 bg-surface-container-low border border-outline-variant rounded-[2.5rem] space-y-8">
                    <div>
                        ...
                        <p className="font-body text-[14px] text-on-surface-variant italic mt-1">Pour soutenir nos équipes en cuisine et en salle</p>
                    </div>
                    <div className="grid grid-cols-4 gap-4">
                        {[10, 15, 20, 25].map(p => (
                            <button 
                                key={p} onClick={() => setTipPercent(p)}
                                className={`py-4 rounded-2xl border-2 font-sans text-xs font-black uppercase tracking-widest transition-all ${tipPercent === p ? 'bg-primary border-primary text-on-primary shadow-xl' : 'bg-surface-container-lowest border border-outline-variant/30 text-on-surface-variant hover:border-primary/50'}`}
                            >
                                {p}%
                            </button>
                        ))}
                ...
                <div className="bg-surface-container-high border border-outline-variant rounded-[2.5rem] p-10 flex flex-col gap-10 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10 space-y-6">
                        ...
                        <div className="space-y-4 font-sans text-[11px] font-black uppercase tracking-widest">
                            ...
                            {tipPercent > 0 && <div className="flex justify-between text-primary"><span>Pourboire</span><span>{tipAmount.toFixed(0)} DH</span></div>}
