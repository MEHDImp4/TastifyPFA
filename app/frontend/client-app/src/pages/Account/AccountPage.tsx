      ...
      <main className="max-w-7xl mx-auto px-client-margin py-12 md:py-24 space-y-24">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
           <div className="lg:col-span-8 bg-white border border-[#2D2424]/5 rounded-[3rem] p-8 md:p-12 flex flex-col md:flex-row items-center gap-10 relative overflow-hidden shadow-xl group">
              ...
              <div className="flex-1 text-center md:text-left space-y-6 z-10">
                 <div>
                    <h1 className="font-serif text-4xl md:text-6xl font-black text-[#2D2424] uppercase italic tracking-tighter m-0 leading-none">{username}</h1>
        ...
        <section className="space-y-12">
            <div className="flex justify-between items-end border-b border-[#2D2424]/10 pb-6">
                <div>
                    <h2 className="font-serif text-4xl font-black text-[#2D2424] italic uppercase tracking-tighter m-0">Privilèges</h2>
                        ...
                        >
                            <div className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    <h4 className="font-serif text-xl font-black text-[#2D2424] uppercase italic">{reward.nom}</h4>
                                    <p className="font-body text-xs text-[#2D2424]/60 italic leading-relaxed">{reward.description}</p>
                                </div>
                            </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-16">
           ...
           <div className="lg:col-span-8 space-y-20">
              ...
              <div className="space-y-10">
                 <div className="flex justify-between items-end border-b border-[#2D2424]/10 pb-6">
                    <div>
                        <h2 className="font-serif text-4xl font-black text-[#2D2424] italic uppercase tracking-tighter m-0">Historique</h2>
                    ...
                    {orders.length > 0 ? orders.map((order, idx) => (
                        ...
                        >
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h4 className="font-serif text-2xl font-black text-[#2D2424] uppercase italic tracking-tight">Commande #{order.id}</h4>
                                    <p className="font-body text-sm text-[#2D2424]/60 italic">
              ...
              <div className="space-y-10">
                 <div className="flex justify-between items-end border-b border-[#2D2424]/10 pb-6">
                    <h2 className="font-serif text-4xl font-black text-[#2D2424] italic uppercase tracking-tighter m-0">Réservations</h2>
                    ...
                    {reservations.length > 0 ? reservations.map((res, idx) => (
                        ...
                        >
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h4 className="font-serif text-2xl font-black text-[#2D2424] uppercase italic tracking-tight">{res.date_reservation}</h4>
                                    <p className="font-body text-sm text-[#2D2424]/60 italic">{res.heure_debut} • Table {res.table}</p>
                                </div>
                            </div>
           ...
           <div className="lg:col-span-4 space-y-10">
              <div className="bg-[#2D2424] text-[#FAF9F6] rounded-[3rem] p-10 space-y-12 shadow-2xl relative overflow-hidden">
                 ...
                 <div className="space-y-4 relative z-10">
                    <h3 className="font-serif text-3xl italic tracking-tight m-0">Menu Privé</h3>
              ...
              {!hasPaidOrders && (
                <div className="p-8 bg-[#D14D1A]/5 border border-[#D14D1A]/20 rounded-[2.5rem] flex items-center gap-5">
                    ...
                    <p className="font-body text-xs text-[#2D2424]/70 italic leading-relaxed">
        ...
        {isReviewModalOpen && hasPaidOrders && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-xl bg-white border border-[#2D2424]/5 rounded-[4rem] p-12 md:p-20 shadow-2xl flex flex-col items-center text-center">
                 ...
                 <h3 className="font-serif text-5xl font-black text-[#2D2424] italic tracking-tighter mb-4">Votre avis nous est précieux.</h3>
                 <p className="text-[#2D2424]/60 font-body italic mb-12">Partagez votre ressenti sur votre dernier moment chez nous.</p>
                 
                 <form onSubmit={handleReviewSubmit} className="w-full space-y-10">
                    <textarea 
                        required value={comment} onChange={(e) => setComment(e.target.value)}
                        className="w-full p-8 bg-[#FAF9F6] border border-[#2D2424]/10 rounded-3xl font-serif text-2xl italic text-[#2D2424] focus:border-[#D14D1A]/30 outline-none transition-all resize-none h-48 placeholder:text-[#2D2424]/10"
